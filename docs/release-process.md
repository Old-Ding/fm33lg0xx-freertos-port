...
- `gpio_blink_mdk`：PB4 LED 周期闪烁，`g_ledTaskCreateStatus == pdPASS`，`g_ledTaskLoopCount` 递增，`g_ledTaskStackHighWaterMark > 0`，`g_freertosFaultCode == 0`。
- `freertos_signal_adc_uart_mdk`：PB4 monitor 正常，PB12 下降沿能唤醒 GPIO task 和 ADC task，UART 输出 ADC 采样值，各任务 stack high-water mark 大于 `0`，`g_freertosFaultCode == 0`。
